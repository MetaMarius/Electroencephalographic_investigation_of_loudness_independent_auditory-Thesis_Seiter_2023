...
import sys
import os
path = "C:/Users/ms26cize/USO_Bachelor_Thesis/last_try/preprocessing/src/eeg_tools"
...
for id in settings.ids:  # loop through all ids
    # if id != '4q868v':
        is_preprocessed = False # utils.check_id(id, settings.root_dir)  # check if subject is already preprocessed
        if is_preprocessed is False:  # if not preprocessed, run loop
            ...
            fig_folder = pathlib.Path(f"C:/Users/ms26cize/USO_Bachelor_Thesis/updated_analysis/preprocessing/data/{id}/figures")
            ...
            epochs = pre.run_pipeline(raw=raw, fig_folder=fig_folder, config=settings.cfg, exclude_event_id=7, root_path=settings.root_dir, id=id)  # run pipeline on raw
