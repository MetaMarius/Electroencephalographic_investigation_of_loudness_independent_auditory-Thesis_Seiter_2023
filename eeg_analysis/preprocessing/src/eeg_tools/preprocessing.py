...
from shifting import shift_epochs_by_onset_delay
...
def run_pipeline(root_path, id, raw, fig_folder, config, ica_ref=None, exclude_event_id=None):
    """
    ...
    if not config:
        ...
        if "epochs" in config:
            ...
            print('Starting creation of metadata...')
            ...
            epochs_not_shifted = mne.Epochs(raw, events=events, metadata=metadata,
                                            **config["epochs"], preload=True)
            print('Shifting and cropping epochs...')
            epochs_not_shifted = epochs_not_shifted['onset_delay_avg.notnull()']
            epochs_shifted = shift_epochs_by_onset_delay(epochs=epochs_not_shifted)
            epochs_shifted.crop(tmin=-0.2, tmax=0.8)
            print('Shifting and cropping finished')
            epochs_shifted.plot(show=False, show_scalebars=False,
                                show_scrollbars=False, n_channels=20)
            plt.savefig(_fig_folder / pathlib.Path("epochs.jpg"), dpi=800)
            plt.close()
        if "rereference" in config:
            epochs_shifted = set_ref(epochs=epochs_shifted, **config["rereference"])
        if "ica" in config:
            epochs_shifted = apply_ICA(
                epochs=epochs_shifted, **config["ica"], reference=ica_ref)
        if "autoreject" in config:
            epochs_shifted = autoreject_epochs(epochs=epochs_shifted, **config["autoreject"])
    # return epochs
